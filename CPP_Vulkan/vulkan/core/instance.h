...
namespace utils::graphics::vulkan
	...
	namespace details
		...
		struct validation_layers
			...
			[[nodiscard]] inline static std::vector<const char*> check_missing()
				{
				if (!enabled) { return {}; }
	...
	struct instance : vk::UniqueInstance
		{
		public:
			instance(const char* application_name, vk::DebugUtilsMessengerCreateInfoEXT debug_utils_messenger_cinfo) : vk::UniqueInstance{create(application_name, debug_utils_messenger_cinfo)}{}

		private:
			inline static vk::UniqueInstance create(const char* application_name, vk::DebugUtilsMessengerCreateInfoEXT debug_utils_messenger_cinfo) noexcept
				{
