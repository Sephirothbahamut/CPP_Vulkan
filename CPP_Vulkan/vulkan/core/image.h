...
namespace utils::graphics::vulkan::core
	...
	class image
		...
		public:
			...
			image(const manager& manager, const create_info& create_info, const vk::Extent3D& extent);
		...
		private:
			...
			vk::UniqueImage        create_image     (const manager& manager, const create_info::image_t& create_info, const vk::Extent3D& extent) const;
			vk::UniqueDeviceMemory allocate_memory  (const manager& manager, const vk::MemoryPropertyFlags required_properties                  ) const;
			vk::UniqueImageView    create_image_view(const manager& manager, const create_info::view_t& create_view_info                        )const;
		};
	}