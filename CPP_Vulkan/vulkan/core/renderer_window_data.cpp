...
namespace utils::graphics::vulkan::core
	...
	renderer_window_data::~renderer_window_data()
		...
		auto& renderer_map{ renderer_ptr->getter(this).window_dependent_data_ptrs() };
	...
	vk::UniqueFramebuffer renderer_window_data::create_framebuffer(const core::manager& manager, const window::window& window, size_t image_index) const
		...
		try
			...
			auto& renderer_image_views = renderer_ptr->getter(this).image_views();
			...
			return 
				{ manager.getter(this).device().createFramebufferUnique(
					...
					{
						.renderPass      { renderer_ptr->getter(this).renderpass() },
	...
	void renderer_window_data::resize(const manager& manager, const window::window& window)
		...
		for (auto& image_create_info : renderer_ptr->getter(this).window_sized_images_create_info())
			{
